...
create table rooted_in_christ.projects (
  id uuid primary key default gen_random_uuid(), title text not null, slug text not null unique,
  description text, location text, status rooted_in_christ.record_status not null default 'draft',
  created_by uuid references auth.users(id), created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
alter table rooted_in_christ.projects enable row level security;
...
create policy "published projects are public" on rooted_in_christ.projects for select using (status = 'published');
...
create policy "ministry editors manage projects" on rooted_in_christ.projects for all using (rooted_in_christ.has_role('editor')) with check (rooted_in_christ.has_role('editor'));
...
grant select on rooted_in_christ.programmes, rooted_in_christ.projects, rooted_in_christ.events to anon, authenticated;
grant insert on rooted_in_christ.contact_enquiries, rooted_in_christ.prayer_requests to anon, authenticated;
grant all on rooted_in_christ.programmes, rooted_in_christ.projects, rooted_in_christ.events, rooted_in_christ.contact_enquiries, rooted_in_christ.prayer_requests, rooted_in_christ.volunteer_applications, rooted_in_christ.partnership_enquiries, rooted_in_christ.donation_records to authenticated;
