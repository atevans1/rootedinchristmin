...
export async function createProgramme(_previous: ProgrammeState, formData: FormData): Promise<ProgrammeState> {
  ...
  const supabase = await createRootedSupabaseServerClient(); if (!supabase) return { ok: false, message: "The secure content service is being prepared. Please try again later." };
  const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""); const { data: { user } } = await supabase.auth.getUser();
  const { error } = await supabase.from("programmes").insert({ title, slug, category, description, status: "published", created_by: user?.id });
  return error ? { ok: false, message: "We could not publish this programme. Please check the title and try again." } : { ok: true, message: "Programme published successfully." };
}
