...
export default async function AdminPage() {
  ...
  return <main id="main-content" className="admin-shell">
    <div className="admin-topbar"><div className="container admin-topbar-inner"><Link href="/" className="admin-brand"><span className="brand-mark"><Icon name="sprout" size={23} /></span><span><strong>Rooted In Christ</strong><small>Ministry administration</small></span></Link><nav aria-label="Admin navigation" className="admin-nav"><Link href="/admin/enquiries">Submissions</Link>{isOwner && <Link href="/admin/users">Users</Link>}<Link href="/admin/logout">Sign out</Link></nav><span className="admin-status">Secure workspace</span></div></div>
