...
export async function submitVolunteer(_previous: VolunteerState, formData: FormData): Promise<VolunteerState> {
  ...
  const supabase = await createRootedSupabaseServerClient(); if (!supabase) return { ok: false, message: "The secure volunteer service is being prepared. Please try again later." };
