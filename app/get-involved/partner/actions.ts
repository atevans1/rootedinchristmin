...
export async function submitPartner(_previous: PartnerState, formData: FormData): Promise<PartnerState> {
  ...
  const supabase = await createRootedSupabaseServerClient(); if (!supabase) return { ok: false, message: "The secure partnership service is being prepared. Please try again later." };
